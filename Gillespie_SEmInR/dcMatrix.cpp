...
Matrix::Matrix(string fileName)
{
	...
	for (unsigned long i=0; i<line.size(); i++) {		//strlen(line.c_str())
		cout<<i<<"=>"<<line[i]<<endl;
	}
	...
	{
		if(line[char_idx]=='\r'||line[char_idx]=='\0' || line[char_idx]=='\n') 
		{
			gotEOL=true;
			num_cols++;
			break;
		...
		// Delimiters can be tab, space, comma
		
		if(line[char_idx]=='\t' || line[char_idx]==' ' || line[char_idx]==',') num_cols++;
	...
	// === Count nb of lines
	// Have to be carefull with newline character that differs accross OS... 
	
	thefile.seekg(0,ios::beg);	
	num_rows = (unsigned long) std::count(std::istreambuf_iterator<char>(thefile), std::istreambuf_iterator<char>(), '\n');	
	cout << "numrow="<<num_rows<<endl;
	
	if (num_rows==0)
	{
		thefile.seekg(0,ios::beg);
		num_rows = (unsigned long) std::count(std::istreambuf_iterator<char>(thefile), std::istreambuf_iterator<char>(), '\r');	
	}
	...
	thefile.seekg(0,ios::beg); // need to return to the begining of the file after counting rows and cols
